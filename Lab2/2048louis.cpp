...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				// info<<"origin"<<move->after_state()<<std::endl;
	 ...
	 */
	void update_episode(std::vector<board>& path_s, std::vector<state>& path_ss, std::vector<board>& path_sss, float alpha = 0.1) const {
		float exact = 0;
		for (path_s.pop_back() /* terminal state */; path_s.size(); path_s.pop_back()) {
			board& before = path_s.back();
			state& move = path_ss.back();
			board& after = path_sss.back();
...
int main(int argc, const char* argv[]) {
	...
	// train the model
	std::vector<board> path_s;
	std::vector<state> path_ss;
	std::vector<board> path_sss;
